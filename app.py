...
VALUE_COL_X = 140
ROW_TOLERANCE = 8
SHEET_HEADERS = ["Gorev No", "Ad Soyad", "Telefon", "Kaynak Dosya"]
...
def satirdan_etiket_deger_ayir(satir):
    ...
    deger = " ".join(t for left, t in items if left >= VALUE_COL_X)
...
def kayitlari_excele_donustur(kayitlar):
    ...
    for kayit in kayitlar:
        ws.append([kayit["Gorev No"], kayit["Ad Soyad"], kayit["Telefon"], kayit["Kaynak Dosya"]])
    buffer = BytesIO()
    wb.save(buffer)
    buffer.seek(0)
    return buffer
